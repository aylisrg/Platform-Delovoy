...
5. **Code Reviewer must flag scope creep.** Scope creep — это незаявленный модуль
   ...
   Добавление модуля вне таблицы → NEEDS_CHANGES. Порога ширины у авто-мерж
   гейта больше нет (убран ADR 2026-08-24 — владелец решил не держать даже
   широкие PR): ловит scope creep теперь только вердикт code-reviewer, а не
   счётчик модулей/строк.
...
- **Auto-merge — только для PR агента и только уровня `auto`.** Мерж в `main`
  ...
  (`scripts/lib/issue-queue.ts`) держит в `hold` ровно один класс — рубильники
  самой автоматики: конфиг, workflow учёта/свипера/ребейзера, реализация гейта,
  контур owner-decisions и промпт `/next-issue`. Это защита не от потери данных,
  а от цикличности: без неё агентский PR мог бы ослабить правило и тем же
  прогоном замержить своё ослабление. **Hold ≠ GitHub-инбокс владельца:**
  ...
  Всё остальное — включая `infra/**`, деплой-workflow'ы, аддитивные **и
  деструктивные** миграции (`DROP TABLE/COLUMN`, `TRUNCATE`, `DELETE FROM`,
  `ALTER TYPE`, `SET NOT NULL`, `DROP CONSTRAINT`), и PR любой ширины (порог
  модулей убран) — мержится автоматически после зелёного CI и PASS от
  `code-reviewer` и `qa-engineer`. С #580 это не конвенция промпта, а машинная
  проверка: гейт требует на PR маркеры обоих вердиктов
  (`npx tsx scripts/issue-queue.ts verdict <PR> code-reviewer|qa-engineer`,
  шаг 6 `/next-issue`), иначе `hold` независимо от CI. Деструктивные миграции и
  широкие PR держали `hold` до ADR 2026-08-24 — владелец решил убрать оба
  порога и принять риск явно; единственная страховка теперь — CI, оба
  ревью-агента, blue-green, снапшот VPS и автооткат (детали и что именно
  принято в риск — ADR `2026-08-24-remove-migration-width-holds`).
