> **УСТАРЕЛ (2026-09-03).** Этот документ описывает палитру `#2563EB`/zinc, заголовки Bold 700 и
> JetBrains Mono — систему, которая в прод так и не попала. Действующая дизайн-система лендинга:
> `DESIGN.md` + `design/tokens.json` + канвас Claude Design (`design/README.md`). Файл оставлен как
> история; в PR внедрения (раздел 8 ТЗ `docs/product/2026-09-03-landing-design-system-brief.md`)
> переезжает в `docs/archive/`. Ничего отсюда в код не брать.
...
|--------|------|-----|-------------|
| **Барбекю Парк** | Зелёный | `#16A34A` | Природа, свежий воздух |
| **Плей Парк** | Фиолетовый | `#7C3AED` | Игры, развлечения |
...
|--------|--------------|-----------|
| Барбекю Парк | `Tent` / `TreePine` | Навигация, карточка модуля |
| Плей Парк | `Gamepad2` | Навигация, карточка модуля |
...
```
┌────────────────────────────────────────────────────────────┐
│ [Логотип]    Барбекю Парк  Плей Парк  Кафе  Парковка    [Войти]  │
...
```
Главная / Барбекю Парк / Бронирование #042
...
Grid 3 колонки (desktop), 2 (tablet), 1 (mobile):
  [Module Card × 5: Барбекю Парк / Плей Парк / Кафе / Парковка / Аренда]
```
...
```
Breadcrumb: Главная / Барбекю Парк

H1: «Барбекю Парк — беседки для отдыха и встреч»
...
Таб «Бронирования»:
  ...
  Пустое состояние:
    ...
    [Забронировать в Барбекю Парке]  [Зайти в Плей Парк]
...
| **P1 — Высокий** | Главная страница, Барбекю Парк, кафе, auth, личный кабинет |
| **P2 — Средний** | Плей Парк, парковка, admin dashboard, admin Барбекю Парк, admin кафе |
