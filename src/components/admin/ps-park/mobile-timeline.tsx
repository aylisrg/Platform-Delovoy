...
export function MobileTimeline({ initialData, initialDate }: Props) {
                ...
                <div className="flex gap-2 overflow-x-auto px-3 pb-3 snap-x">
                  {slots.map((s) => {
                    ...
                    if (activeBooking) {
                      ...
                      if (!isStart) return null; // skip, only render at booking start cell and span
                      // Compute visual span — count number of half-hour slots from start to end
                      ...
                      const widthPx = span * 72 + (span - 1) * 8; // 72px slot + 8 gap
