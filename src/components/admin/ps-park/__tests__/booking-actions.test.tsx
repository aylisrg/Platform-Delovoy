...
function renderActions(currentStatus: BookingStatus) {
  render(
    <BookingActions
      ...
      resourceName="Стол №1"
      timeLabel="01.09.2026, 10:00–12:00"
    />
  );
}
...
describe("BookingActions (ps-park) — Заехал / Не пришёл (#436)", () => {
  beforeEach(() => {
  ...
  // NO_SHOW-бронь не входит в ACTIVE_BOOKING_STATUSES и не видна в
  // расписании — единственное место, где до неё можно дотянуться, это
  // история броней, которая рендерит именно этот компонент.
  it("«Заехал» доступен и для NO_SHOW — поздний заезд опоздавшего гостя из истории брони", async () => {
    vi.mocked(fetch).mockResolvedValue(
      jsonResponse({ success: true, data: { id: "booking-1", status: "CHECKED_IN" } })
    );
    ...
    await waitFor(() => expect(refreshMock).toHaveBeenCalled());
    expect(vi.mocked(fetch).mock.calls[0][0]).toBe("/api/ps-park/bookings/booking-1/checkin");
  });

  it("показывает BOOKING_CONFLICT, если стол успели пересдать при позднем заезде", async () => {
    vi.mocked(fetch).mockResolvedValue(
      jsonResponse(
