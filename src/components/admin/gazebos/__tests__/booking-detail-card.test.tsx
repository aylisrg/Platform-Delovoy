...
describe("GazeboBookingDetailCard", () => {
  ...
  describe("Заехал / Не пришёл (#436)", () => {
    ...
    it("показывает BOOKING_CONFLICT при позднем заезде на уже занятый слот", async () => {
      vi.mocked(fetch).mockResolvedValue(
        jsonResponse(
    ...
    it("не показывает «Заехал»/«Не пришёл» для уже заехавшего гостя — только «Завершить»", () => {
      renderCard({ status: "CHECKED_IN" });

      // "Заехал" остаётся текстом бейджа статуса — проверяем отсутствие
      // именно КНОПКИ, а не текста на странице вообще.
      expect(screen.queryByRole("button", { name: "Заехал" })).toBeNull();
      expect(screen.queryByText("Не пришёл")).toBeNull();
      expect(screen.getByText("Завершить")).toBeTruthy();
    });

    it("бейдж статуса показывает «Заехал» для CHECKED_IN, а не «Подтверждена»", () => {
      renderCard({ status: "CHECKED_IN" });

      expect(screen.getByText("Заехал")).toBeTruthy();
