...
export function GazeboBillModal({
  bill,
  isOpen,
  onClose,
  onConfirm,
  ...
  const discountAmount =
    discountPercent > 0 ? Math.round(originalTotal * discountPercent / 100) : 0;
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional reset on amount change
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-white shadow-2xl mx-4 overflow-hidden max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 pt-5 pb-3">
          <h2 className="text-base font-semibold text-zinc-900">Завершение брони беседки</h2>
        ...
        <div className="px-6 pb-4">
          <div className="border border-zinc-200 rounded-xl overflow-hidden">
            {showDiscount && discountPercent > 0 && (
              <div className="border-b border-amber-200 px-4 py-2.5 bg-amber-50/50">
        ...
        <div className="flex gap-3 px-6 pb-5">
          ...
          >
            {confirming ? "Завершение..." : "Завершить бронь"}
          </button>
        </div>
      </div>
    </div>
