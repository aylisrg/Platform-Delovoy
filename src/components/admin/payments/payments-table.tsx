...
export type PaymentsTableProps = {
  /** Полный URL вебхука для ЛК ЮKassa — только для SUPERADMIN, иначе null. */
  webhookUrl: string | null;
  /** Заданы ли YOOKASSA_SHOP_ID/SECRET_KEY в окружении сервера. */
  configured: boolean;
};

export function PaymentsTable({ webhookUrl, configured }: PaymentsTableProps) {
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        {!configured && (
          <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            Онлайн-оплата не настроена: на сервере не заданы ключи ЮKassa
            (YOOKASSA_SHOP_ID / YOOKASSA_SECRET_KEY). POS-приём работает как обычно.
          </div>
        )}

        {webhookUrl && (
          <details className="mb-4 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900">
            <summary className="cursor-pointer font-medium">
              Настройка вебхука ЮKassa (для суперадмина)
            </summary>
            <p className="mt-2">
              Вставьте этот URL в ЛК ЮKassa → Интеграция → HTTP-уведомления и отметьте
              события: payment.succeeded, payment.waiting_for_capture, payment.canceled,
              refund.succeeded.
            </p>
            <p className="mt-2 break-all rounded bg-white px-2 py-1 font-mono text-xs">
              {webhookUrl}
            </p>
            <button
              type="button"
              onClick={() => navigator.clipboard.writeText(webhookUrl)}
              className="mt-2 rounded-lg border border-blue-300 px-3 py-1 text-xs hover:bg-blue-100"
            >
              Скопировать URL
            </button>
          </details>
