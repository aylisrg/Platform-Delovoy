...
export default async function NedelovoyTasksPage() {
  ...
  const tasks = await prisma.managerTask.findMany({
    where: { moduleSlug: "nedelovoy" },
    orderBy: [{ status: "asc" }, { createdAt: "desc" }],
    take: 200,
  });

  const contractIds = [
    ...new Set(tasks.map((t) => t.contractId).filter((v): v is string => !!v)),
  ];
  const contracts = contractIds.length
    ? await prisma.rentalContract.findMany({
        ...
        include: {
          tenant: {
            select: { companyName: true, contactName: true, phone: true, email: true },
          },
    ...
    : [];
  const paymentIds = [
    ...new Set(tasks.map((t) => t.paymentId).filter((v): v is string => !!v)),
  ];
  ...
  return (
    <>
      <AdminHeader title="НеДеловой — задачи менеджера" />
