...
const inquiryStatusVariant: Record<
  InquiryStatus,
  "warning" | "success" | "default" | "info" | "danger"
> = {
  NEW: "warning",
  IN_PROGRESS: "info",
  CONVERTED: "success",
...
export default async function NedelovoyManagerPage() {
  const session = await auth();
  if (!session?.user?.id) forbidden();
  const hasAccess = await hasAdminSectionAccess(session.user.id, "nedelovoy");
  if (!hasAccess) forbidden();

  const now = new Date();
  const overdueTasksCount = await prisma.managerTask.count({
    where: { moduleSlug: "nedelovoy", status: "OPEN", type: "OVERDUE_PAYMENT" },
  });
  ...
  const upcomingCount = await prisma.rentalPayment.count({
    where: {
      paidAt: null,
      dueDate: { lte: in7Days },
      contract: { parkSlug: PARK, status: { in: ["ACTIVE", "EXPIRING"] } },
    },
  });
  ...
  const [
    ...
    prisma.rentalContract.findMany({
      where: { parkSlug: PARK, status: { in: ["ACTIVE", "EXPIRING"] } },
    ...
    prisma.rentalContract.count({
      where: { parkSlug: PARK, startDate: { gte: monthStart } },
    }),
    prisma.rentalContract.findMany({
      where: { parkSlug: PARK, status: { in: ["ACTIVE", "EXPIRING"] } },
      select: { monthlyRate: true },
  ...
  for (const o of offices) {
    if (!buildingStats.has(o.building))
      buildingStats.set(o.building, { total: 0, occupied: 0 });
  ...
  return (
    <>
      <AdminHeader title="НеДеловой — CRM" />
      <div className="p-6 lg:p-8">
        <div className="mb-6 flex flex-wrap gap-2 text-sm">
          <Link
            href="/admin/nedelovoy/payments"
          ...
          <Link
            href="/admin/nedelovoy/email-log"
            className="inline-flex items-center rounded-full border border-zinc-200 bg-white px-3 py-1.5 hover:border-zinc-300"
          >
            Журнал писем
          </Link>
          <Link
            href="/admin/nedelovoy/email-templates"
            className="inline-flex items-center rounded-full border border-zinc-200 bg-white px-3 py-1.5 hover:border-zinc-300"
          >
            Шаблоны писем
          </Link>
          <Link
            href="/admin/nedelovoy/notification-settings"
            className="inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-white px-3 py-1.5 hover:border-zinc-300"
          >
            Настройки рассылок
            {settings && !settings.autoSendEnabled && (
              <span className="text-[11px] font-semibold text-amber-600">· выкл.</span>
            )}
          </Link>
        </div>
        <RentalTabs>
                ...
                <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
                  <StatusWidget
                    ...
                    description={`${Math.round(
                      (occupiedOffices / (offices.length || 1)) * 100
                    )}% занятость`}
                  />
                  <StatusWidget
                    title="Выручка/мес"
