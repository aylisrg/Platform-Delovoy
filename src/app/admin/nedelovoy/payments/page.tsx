...
export default async function NedelovoyPaymentsPage() {
  ...
  const payments = await prisma.rentalPayment.findMany({
    where: {
      ...
      contract: { parkSlug: "nedelovoy", status: { in: ["ACTIVE", "EXPIRING"] } },
    },
    include: {
      contract: {
        include: {
  ...
  return (
    <>
      <AdminHeader title="НеДеловой — ожидают оплаты" />
      <div className="p-6 lg:p-8 max-w-6xl">
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-zinc-500 mt-1">{payments.length} позиций</p>
          </CardHeader>
          <CardContent>
