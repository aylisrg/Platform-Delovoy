...
export default async function NedelovoyEmailLogPage() {
  ...
  return (
    <>
      <AdminHeader title="НеДеловой — журнал писем" />
                  ...
                  {logs.map((l) => (
                    <tr key={l.id} className="border-b border-zinc-50">
                      ...
                      <td className="py-2 pr-3 text-zinc-600">
                        {l.tenantId
                          ? (tenantMap.get(l.tenantId)?.companyName ?? "—")
                          : "—"}
                      </td>
                      <td className="py-2 pr-3 text-zinc-600 max-w-xs truncate">
                        {l.to.join(", ") || "—"}
                      </td>
                      <td className="py-2 pr-3 text-zinc-900 max-w-md truncate">
                        {l.subject}
                      </td>
                      <td className="py-2">
                        {l.status === "SENT" ? (
