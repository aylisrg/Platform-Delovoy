...
import { InventoryInteractive } from "@/components/admin/inventory/inventory-interactive";
...
export default async function InventoryPage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-zinc-900">Склад</h1>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div className={`rounded-xl border p-4 shadow-sm ${lowStockCount > 0 ? "border-amber-200 bg-amber-50" : "border-zinc-200 bg-white"}`}>
          <p className="text-xs font-medium text-zinc-500 uppercase tracking-wide">Мало / нет</p>
          <p className={`mt-1 text-2xl font-bold ${lowStockCount > 0 ? "text-amber-700" : "text-zinc-900"}`}>
            {lowStockCount}
          </p>
          {outOfStockCount > 0 && (
        ...
        {skus.length === 0 ? (
          <div className="px-6 py-12 text-center">
            ...
            <p className="text-xs text-zinc-300 mt-1">
              Запишите первый приход ниже — товар появится здесь автоматически.
            </p>
          </div>
        ) : (
      ...
      {/* Receive form + receipts history (client-side interactive) */}
