...
import { listExpenses, createExpense } from "@/modules/management/service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const parsed = expenseFilterSchema.safeParse(params);
    if (!parsed.success) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const record = await createExpense(parsed.data, session.user.id);
