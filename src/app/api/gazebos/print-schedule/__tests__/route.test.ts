// issue #668: печатный лист дня — тот же гейт, что и у GET /api/gazebos/timeline.
...
function makeRequest(query: string) {
  return new NextRequest(`http://localhost/api/gazebos/print-schedule${query}`);
}
...
describe("GET /api/gazebos/print-schedule", () => {
  it("возвращает список для MANAGER с доступом к модулю", async () => {
    ...
    expect(mockGetPrintableDaySchedule).toHaveBeenCalledWith("gazebos", "2026-06-15", false);
  });
  ...
  it("includeCancelled=true передаётся в сервис как true (AC-4)", async () => {
    ...
    expect(mockGetPrintableDaySchedule).toHaveBeenCalledWith("gazebos", "2026-06-15", true);
  });

  it("без параметра includeCancelled по умолчанию false", async () => {
    await GET(makeRequest("?date=2026-06-15"));
