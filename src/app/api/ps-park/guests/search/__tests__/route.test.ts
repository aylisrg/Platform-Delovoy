...
function makeRequest(query: string) {
  return new NextRequest(`http://localhost/api/ps-park/guests/search${query}`);
}
...
describe("GET /api/ps-park/guests/search (issue #666)", () => {
  it("happy path: возвращает найденных гостей", async () => {
    ...
    expect(mockSearchGuestsByPhone).toHaveBeenCalledWith("ps-park", "999");
  });
  ...
  // AC-4: гейт — доступ к модулю ps-park, а не к CRM-разделу clients.
  it("гейтится через requireAdminSection('ps-park'), не через 'clients'", async () => {
    await GET(makeRequest("?phone=999"));

    expect(mockRequireAdminSection).toHaveBeenCalledWith(expect.anything(), "ps-park");
  });

  it("MANAGER без доступа к модулю ps-park — requireAdminSection отклоняет", async () => {
