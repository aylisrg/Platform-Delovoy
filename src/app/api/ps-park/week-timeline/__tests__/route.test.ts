// issue #740: недельный вид — тот же гейт, что у GET /api/ps-park/timeline, плюс рейт-лимит.
...
function makeRequest(query: string) {
  return new NextRequest(`http://localhost/api/ps-park/week-timeline${query}`);
}
...
describe("GET /api/ps-park/week-timeline", () => {
  it("MANAGER с доступом к секции получает неделю; часы и минимум — из настроек модуля", async () => {
    ...
    expect(mockGetWeekTimeline).toHaveBeenCalledWith("ps-park", "2030-06-19", {
      openHour: 8,
      closeHour: 23,
      minBookingHours: 2,
    });
