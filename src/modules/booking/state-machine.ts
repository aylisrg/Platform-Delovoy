 ...
 *
 * ⚠️ Обратный переход `NO_SHOW → CHECKED_IN` (опоздавший гость) при этом
 * **не перепроверяет занятость**: `checkInBooking()` в обоих модулях делает
 * голый `update` без конфликт-чека. Если слот успели пересдать, реактивация
 * неявки создаёт двойную бронь. Это не следствие состава константы, а
 * самостоятельная дыра — трекается в #478.
 *
