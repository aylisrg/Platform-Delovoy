 ...
 * - `CANCELLED` — брони нет;
 * - `NO_SHOW` — гость не приехал, слот честно отдаётся другим. Включать его сюда
 *   нельзя: иначе старая неявка вечно блокировала бы ресурс от пересдачи.
 *
 * Обратный переход `NO_SHOW → CHECKED_IN` (опоздавший гость) перепроверяет
 * занятость отдельно: `checkInBooking()` в обоих модулях заворачивает эту
 * ветку в транзакцию с `lockSlot()` + конфликт-чеком (#478), тем же паттерном,
 * что и создание брони. `CONFIRMED → CHECKED_IN` в этом не нуждается — бронь
 * всё это время занимала слот и из активных не выпадала.
 *
