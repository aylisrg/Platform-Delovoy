...
export const createBookingSchema = z.object({
  ...
  // Контакт для чека 54-ФЗ и адрес подтверждения бронирования. У гостя
  // обязателен: без него письмо с номером редакции оферты и ссылкой на
  // управление бронью доставить некуда (п. 4.6 оферты, ТЗ §7). Требование
  // гостевой ветки проверяет route handler — у залогиненного берётся профиль.
  email: z.string().email("Некорректный email").max(200).optional(),

  // === Акцепт оферты (ТЗ §5, Приложение № 3 оферты) ===
  // z.literal(true), а не boolean: договор заключается ТОЛЬКО с отметкой.
  // Дефолта здесь нет и быть не может — согласие приходит из тела запроса и
  // никогда не проставляется схемой (требование #9.1).
  acceptOffer: z.literal(true, {
    message: "Чтобы продолжить, подтвердите согласие с условиями оферты",
  }),
  // Редакция, которую клиент видел в момент нажатия «Оплатить»; сервер сверит
  // её с действующей.
  offerVersionSlug: z.string().min(1).max(32),
  // Реклама — отдельное добровольное согласие. `false` по умолчанию — это
  // отсутствие согласия, а не согласие по умолчанию.
