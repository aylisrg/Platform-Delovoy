...
> **Итог QA-аудита 2026-08-10** (`docs/qa-reports/2026-08-10-booking-relaunch-audit.md`): все кандидаты подтверждены (10/10 P0, 5/5 P1), вердикт FAIL до фиксов. По рекомендации QA в волну P0 дополнительно подняты три пункта из P1: атрибуция админ-броней гостю (№1 ниже), пагинация истории (№2) и Zod-валидация PATCH с `min(0)` для сумм (№4) — отрицательные суммы искажают кассовую разбивку смены. Новые находки QA: CHECKED_IN-слот рендерится «свободным» в сетке (следствие P0-2), хардкод минимума 4 ч в квик-попапе игнорирует настройку `minBookingHours`, в фильтре истории нет опции CHECKED_IN. Отдельно: `package-lock.json` рассинхронизирован — `npm ci` падает на чистом окружении (отдельный chore-issue).
...
## 4. Реестр задач (GitHub Issues, созданы 2026-08-10)

**Волна P0 (блокеры):** #423 soft-delete gazebos · #424 CHECKED_IN в конфликт-чеках/таймлайне · #425 кнопки карточки таймлайна · #426 webapp DELETE мимо ядра · #427 bot cancel-booking лжёт об отмене · #428 session-ending-alert security · #429 TOCTOU-гонка · #430 атрибуция админ-броней · #431 пагинация истории · #432 Zod PATCH статуса. Плюс #445 package-lock (chore, блокирует холодный CI).

**Волна P1:** #433 GCal+уведомления при переносе · #434 мёртвые настройки часов · #435 возврат часов абонемента · #436 UI check-in/no-show (+NEW-2/3) · #437 админ-брони в Telegram-канал · #438 поиск по имени/телефону · #439 смена даты/беседки в форме · #440 no-show порог · #441 бэкфилл прошлых броней (по решению владельца).

**P2 (сводные):** #442 UX календаря · #443 блокировка слотов (нужен PRD) · #444 hardening/чистка/тест-долг.

Порядок исполнения P0: #423 → #424 → #429 (конфликт-чеки, последовательно) → #425; #426/#427/#428/#430/#431/#432 — параллельно.

## 5. Процесс исполнения
