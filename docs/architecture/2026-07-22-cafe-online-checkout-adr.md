...
**Дополнение 2026-08-12 — минимум полей, контакт только email.** Своей кассы у
кафе нет, ЮKassa — единственная фискализация, а «Чеки от ЮKassa» доставляют чек
**только на почту (СМС недоступна)**. Поэтому:
- `checkoutSchema`/`CheckoutInput` больше не принимают `customerPhone`, а
  `resolvePaymentEmail` не подставляет телефон профиля: принятый телефон давал
  оплаченный заказ с недоставленным чеком — молчаливое нарушение 54-ФЗ.
- Форма проверяет адрес общей схемой `receiptEmailSchema` **до** запроса:
  опечатка больше не доезжает до `createCheckout`, который создавал заказ и тут
  же отменял его (мусорные `CANCELLED` в БД и статистике).
- Поля `deliveryTo`/`comment` убраны с быстрого пути в раскрывающийся блок
  «Принести в офис» — у стойки чекаут это одно поле и кнопка. В схеме, БД и
  авторизованном `/api/cafe/order` оба поля остались без изменений.
- Ввод email сведён к одному тапу: `autoComplete="email"`, запоминание в
  `localStorage` (`cafe-email-v1`), подстановка почты залогиненного из сессии.

Чекаут **без единого поля** достижим только при собственной кассе в кафе:
тогда `YOOKASSA_RECEIPTS_ENABLED=false` убирает поле само, без правок кода.
Держать обе фискализации одновременно нельзя — задвоение выручки в ФНС.
