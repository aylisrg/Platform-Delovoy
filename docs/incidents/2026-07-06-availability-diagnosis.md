...
## Дополнение 2026-07-18 — повторная диагностика и закрытие инцидента

Жалоба «сайт опять не загружается» повторилась. Проведена полная повторная
диагностика через `timeweb-manage.yml` (`server-status`, `server-logs`,
новый `ops-diagnose`).

### Что изменилось с 2026-07-06

- **Сервер апгрейжен**: `delovoy-park-prod` теперь **2 CPU / 4 GB** (preset
  2453) — гипотеза «сервер вдвое меньше задуманного» больше не актуальна.
- **OOM не подтверждается**: за 7 дней ни одного OOM-события в dmesg/journal;
  все контейнеры `restarts=0, oomkilled=false`; память 1.1/3.9 GB, swap 2G
  почти не используется, load average 0.11.
- app/bot перезапускались только деплоем (14 июля), НЕ крашами.

### Актуальные причины симптома «сайт не загружается»

1. **Деплой-даунтайм**: `docker compose up -d --no-deps app` убивает старый
   контейнер, новый проходит тяжёлый entrypoint (prisma generate/migrate/seed)
   1–3 минуты — всё это время nginx отдаёт 502. Деплои случаются пару раз в
   неделю → «циклический» характер жалоб.
2. **Стейл-клиенты после деплоя**: открытые вкладки со старым бандлом ловят
   `Failed to find Server Action` / 404 на чанках до перезагрузки страницы.
3. Шум сканеров (`Server Action "dontcare"/"x"/"y"`) маскирует реальные ошибки
   в логах, но на пользователей не влияет.

### Сделано (ветка claude/site-loading-failures-fdcubm)

- **Site Watchdog** (`.github/workflows/site-watchdog.yml` +
  `scripts/watchdog-remediate.sh`): внешняя проверка каждые ~5 мин,
  авто-восстановление по SSH (зомби → nginx → restart app → compose up),
  Telegram-алерты, issue `site-down` с авто-закрытием.
- **Ежедневный бэкап реально включён**: аудит показал, что cron бэкапа не был
  зарегистрирован вовсе; deploy.yml теперь копирует скрипты на VPS и
  регистрирует cron `30 1 * * *` UTC; `backup-db.sh` научился работать через
  `docker exec` (на хосте нет pg_dump), `-Z 9 → -Z 3`, `nice/ionice`.
- **Лимиты под 4 GB**: app 1G → 1536M, `NODE_OPTIONS` 768 → 1152;
  `connection_limit=10` (app) / `3` (bot) + `pool_timeout=20` в DATABASE_URL.
- **docker-cleanup**: ежедневный prune → еженедельный (после деплоя остаётся).
- **Этап 3** (bounded-запросы clients, кэш getSystemMap, hardening
  /api/health) — реализован в этой же ветке.

### Остаётся (осознанные решения)

- `delovoy-agent` работает на прод-VPS — решение владельца оставить
  (памяти после апгрейда достаточно).
- Zero-downtime деплой (blue-green: новый контейнер поднимается рядом,
  трафик переключается после health check) — кандидат на отдельный PR;
  устранит причину №1 полностью.
