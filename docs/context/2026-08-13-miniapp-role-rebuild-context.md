...
Реализация: фундамент (координатор) + 4 параллельных трека (senior-developer агенты), интеграция без конфликтов.

- **Фундамент до фан-аута:** обе миграции + schema закоммичены первыми (сняли единственный межтрековый конфликт), затем auth/capabilities/navigation + дизайн-система (14 токенов, UI-кит, фикс вложенных html/body) — треки строили поверх готового.
- **ApiFetchError** (code/status/data) добавлен в apiFetch фундаментом — иначе 402-штраф неразличим на клиенте.
- Трек D: тело релиз-анонса переведено на plain text (TelegramChannel сам экранирует HTML — легаси-разметка дала бы &lt;b&gt; в тексте); guard-джоб deploy.yml — fail-open при сбое gh api.
- Трек C: путь 2b дедупит самоподписанных против explicit-списка кодом (не полагаясь на 5-мин dedup-окно); гонка автопровижининга канала закрыта перехватом P2002; категория payments/avito/system в delivery-подсказке не обещает групповой чат.
- Трек B1: unreadCount по новостям ограничен сканом 100 кампаний; bc:-id в /feed/read двигает watermark (иначе новость нельзя отметить прочитанной).
- Каталог: категория «Авито» вместо «Обратная связь» из ADR — секции feedback нет в ADMIN_SECTIONS, событий feedback.* нет в EVENT_ROUTING; честный вариант без мёртвых тумблеров.
- Review-итерация: 2 MAJOR исправлены (guard curl в deploy.yml; rate limit до ре-чека роли в notification-center — безтокенный путь лимитируется public-тиром по IP) + 3 минорных QA (401-UX «Моих броней», русская строка, 429-тест orders).
- Итог: QA PASS (43/44 AC, 44-й — визуальная приёмка владельцем), Review PASS (после итерации), 3382 теста зелёные, tsc/lint чистые. PR остаётся draft до визуальной приёмки владельцем (AC-7.5).
