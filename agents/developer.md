...
8. **AC-трассируемость.** Если фича реализуется по PRD (`docs/requirements/*-prd.md`) —
   тест, покрывающий конкретный AC, обязан содержать маркер: `// AC-N` в комментарии
   рядом или `AC-N` в названии `it(...)`/`test(...)`. Структурная проверка —
   `eval/checks/traceability.ts` (issue #585), пока в режиме отчёта в CI (не блокирует).
