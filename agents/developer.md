...
- [ ] **Финальная сверка**: перечитаны все acceptance criteria, каждый из них покрыт кодом и тестом

---

## Правила

1. **Сверяйся с первоначальной задачей.** Перед реализацией перечитай user stories и acceptance criteria от PO, а также ADR от Architect. Перед коммитом проверь: каждый AC реализован? Если в процессе появились дополнительные "улучшения" — они должны быть явно запрошены, а не добавлены по инициативе. Если AC не покрыт кодом — это незаконченная задача, не коммит.

2. **Тесты вместе с кодом.** Каждый коммит с новым кодом включает тесты.
