...
### Added
- `agents/analytics.md` — раздел «Недельный отчёт по воронке (эпик #583)»:
  правила режима US-2/US-3 (issue #726) — источник чисел только сгенерированный
  отчёт, агент пишет только прозу («Интерпретация» + тексты гипотез), максимум
  3 гипотезы в неделю, baseline подставляет CLI, «данных мало» — законный
  результат, никаких PII в тексте, гипотеза не задача (лейбл `auto:hypothesis`,
  ход даёт только владелец через `issue-queue.ts promote`). Backward
  compatible — существующие правила read-only/no-PII не менялись.

### Agent versions bumped
- `analytics` → 1.2.0 (новая секция, backward compatible)
...
### Changed
- `agents/reviewer.md` — политика PR-гранулярности: размер батч-PR — не основание
  для NEEDS_CHANGES; scope creep — это код без своей задачи/PRD, а не большой дифф.
  Новый пункт чеклиста Scope Creep (у каждого изменения батча — своя issue/PRD)
  и уточнение правила 6. Синхронно с CLAUDE.md Scope guard #3/#5 («микро-PR не
  делаем — компонуем и катим», указание владельца).

### Agent versions bumped
- `reviewer` → 1.4.0 (batch-PR policy, backward compatible; c 1.1.0 сразу на
  1.4.0 — заголовки ченджлога монотонны, 1.2.0/1.3.0 заняты параллельными
  bump'ами `developer`/`qa`)
...
### Added
- `qa.md` — раздел «После вердикта: обновление qa-patterns.md»: обязательный шаг дозаписи 1–3 строк в `.claude/feedback/qa-patterns.md` в формате «паттерн → как избегать» после каждого QA-вердикта, плюс правило ротации при переполнении секции/файла (issue #575).
- `.claude/agents/qa-engineer.md` — зеркальная, сжатая версия того же шага в numbered-процессе агента.
- `.claude/feedback/qa-patterns.md` — засеяны 5 пустовавших секций «Частые ошибки» 8 реальными паттернами из истории `docs/qa-reports/`.

### Agent versions bumped
- `qa` → 1.3.0 (новая секция, backward compatible; версия сдвинута с 1.2.0 на 1.3.0 при
  ребейзе на `main` — 1.2.0 уже занята одновременным bump'ом `qa` в записи ниже,
  issue #585, смержено первым)
