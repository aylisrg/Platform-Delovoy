...
export default defineConfig({
  ...
  expect: {
    toHaveScreenshot: {
      threshold: 0.7,
      // Абсолютный (не ratio!) потолок — не масштабируется с размером
      // страницы, поэтому не маскирует локальную поломку вроде AC1 issue
      // #579 (смена цвета CTA-кнопки ~13000px+, что заметно больше потолка
      // на любом из 9 сценариев, включая мобильный viewport). Калибровано
      // по факту реального CI при threshold=0.7: остаточный AA-шум —
      // максимум ~6969px (home-mobile), у остальных 8 страниц заметно
      // меньше (условная граница с запасом ~15%).
      maxDiffPixels: 8000,
    },
  },
  use: {
