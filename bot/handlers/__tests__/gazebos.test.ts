...
import { registerGazeboHandlers } from "../gazebos";
...
function collectHandlers() {
  ...
  registerGazeboHandlers(fakeBot as never);
...
// #534: availability.resource.name — админский ввод (название беседки),
// сообщение уходит с parse_mode:"HTML".
describe("gz_date callback — экранирование resource.name (#534)", () => {
  it("экранирует resource.name перед подстановкой в HTML-сообщение", async () => {
    vi.mocked(fetch).mockResolvedValue(
      jsonResponse({
        ...
        data: [
          {
            resource: { name: "<b>Беседка</b><script>alert(1)</script>" },
    ...
    const handler = callbacks.find((c) => c.pattern.toString().includes("gz_date"))!.handler;
    const ctx = makeCtx({ match: [":gz_date:res-1:2026-09-01", "res-1", "2026-09-01"] });
