...
- Обязательное текстовое поле (причина/комментарий), которое сервер требует ввести перед сохранением, рендерится в UI только внутри условия, завязанного на *другое* поле (`{discrepancy !== 0 && <текст note>}`) → в легитимной комбинации состояний (коррекция кассы, где исправленная сумма точно совпала с расчётной — `discrepancy === 0`) текст сохранён в БД/AuditLog, но не показан ни на одном экране, хотя форма заставила его ввести → рендерить обязательные user-supplied поля по факту их непустоты (`note && ...`), не по состоянию поля, которое лишь иногда с ним коррелирует (PR #754).
- Новый обработчик навигации/переключения вида (клик по пустой ячейке недельного вида → переход в дневной) сбрасывает основные данные (`loadTimeline`), но не item-selection state (`selectedBooking`/override), который уже сбрасывает соседний обработчик того же компонента (`handleBookingStatusChanged`) → карточка/панель от предыдущего выбора остаётся видимой поверх нового представления с несвязанными данными → при добавлении обработчика, меняющего представление/данные, сверять его со списком state, которые сбрасывают уже существующие соседние обработчики того же компонента, а не полагаться на явную формулировку AC (issue #740/#741).

- Regex/строковый литерал в `.ts`-файле набран буквальными control-байтами (NUL, DEL) вместо текстовых escape-последовательностей (`\x00` как 4 печатных символа, а не как реальный байт) → `git diff`/GitHub PR показывают файл как бинарный («Binary files ... differ»), а мандатный security-грep из `agents/SECURITY.md` (без `-a`) молча пропускает его контент («binary file matches») — ревьюер физически не видит код, который одобряет → набирать спецсимволы в regex только текстовыми escape-последовательностями, после правки сверять `git diff -- <файл>` — должен быть построчным, не `Binary files ... differ` (issue #916).
...
- Параметр восстановления состояния после логина (`callbackUrl`) прокидывается через `useSearchParams()`/замыкание для одних провайдеров (пароль, поллинг без ухода со страницы, OAuth — там redirect несёт NextAuth сам), но провайдер с почтовым round-trip (magic-link: письмо → клик → серверный 302 на новую загрузку страницы) не хранит и не прокидывает его ни на одном хопе → значение тихо теряется именно на этом способе входа, хотя тесты и ревью формально считают «email» одним пройденным кейсом → для параметра, который должен пережить внешний async round-trip (email/SMS-ссылка), грепом проверять его имя по всей цепочке (schema → generate → send → verify-redirect), а не полагаться на то, что браузер вернётся с тем же query string (issue #916).
...
- Sanitizer открытого редиректа с несколькими ветками нормализации (путь vs абсолютный URL) применяет anti-protocol-relative проверки (`//`, `\`, control-байты) только в путевой ветке — ветка «абсолютный URL → схлопнуть до pathname» возвращает `url.pathname` напрямую, а проверка `url.origin === ownOrigin` пропускает путь мимо: `https://own-origin//evil.com` имеет свой origin, но `pathname === "//evil.com"` → тот же protocol-relative обход, который остальная функция ловит → каждую ветку нормализатора либо звать рекурсивно через общую путевую проверку, либо выносить anti-open-redirect проверки в отдельную функцию, звать из всех веток без исключений (issue #916).
...
| unknown | issue-669-qa-report.md | 0 | rbac, api, typescript, tests |
...
| 2026-09-18 | 2026-09-18-for-team-qa-report.md | 17 | rbac, api, tests, scope_creep |
